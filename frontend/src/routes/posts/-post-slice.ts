...
export const postAdded = createAsyncThunk<Post, CreatePostPayload>(
  "posts/addPost",
  async ({ authorId, title, content, visible }, thunkAPI) => {
    try {
...
export const togglePostVisibility = createAsyncThunk<Post, string>(
  "posts/toggleVisibility",
  async (id, thunkAPI) => {
    try {
      // First, get the current state
      const state = thunkAPI.getState() as RootState;
      const post = state.posts.posts.find((p) => p._id === id);

      if (!post) {
        throw new Error("Post not found");
      }

      // Toggle the visibility (use opposite of current value)
      const newVisibility = !post.visible;
      ...
      const axiosError = error as AxiosError<{ error: string }>;
      return thunkAPI.rejectWithValue(
        axiosError.response?.data?.error || "Failed to toggle post visibility"
      );
    }
  }
);
